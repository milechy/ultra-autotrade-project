...
## 2026-06-14 i18n wave マージが機能 PR のキーを消す / next-intl は生キーを黙って描画する

**何が起きたか**: staging で緊急停止ボタンに生キー `Liff.home.emergencyStop` が表示された (PR #730 で修正)。原因は #712 が追加した `Liff.home.emergencyStop` 系 10 キーが、後続の i18n wave 系 PR (`messages/ja.json`・`en.json` を**全体上書き**) に巻き込まれて消失していた。`layout.tsx` 重複 import (#726) と同根の「wave マージが他の変更を壊す」パターン。

**なぜ CI を素通りしたか**: next-intl はキー未解決時に**例外を投げず生キーをそのまま描画**するため、`next build` も `tsc` も通過してしまう (型・ビルドでは検出不能)。ユーザーに直接見えるデグレなのに無検出。
