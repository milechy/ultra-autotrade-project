    ...
    >
      ...
      <div className="flex flex-col gap-1 text-sm">
        <div className="flex justify-between">
          ...
          <span>${state.deposit_usd.toLocaleString()}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">借入</span>
          <span>${state.borrow_usd.toLocaleString()}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">年利</span>
          <span>{state.net_apy.toFixed(2)}%</span>
        </div>
        <div className="flex justify-between">
        ...
        {state.health_factor !== null && (
          <div className="flex justify-between">
            ...
            <span>{state.health_factor.toFixed(2)}</span>
          </div>
        )}
      </div>
    </div>
    ...
    <Card className={className}>
      <CardHeader className="pb-2">
        <CardTitle className="text-base">
          {data.action_type} ${data.amount_usd.toLocaleString()} の影響
        </CardTitle>
      </CardHeader>
