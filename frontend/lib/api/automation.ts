...
export async function fetchAutomationStatus(): Promise<AutomationStatus> {
  ...
  try {
    ...
    if (e?.status === 404) {
      // フォールバック: 旧パス
      return await getJson<AutomationStatus>("/api/status");
    }
    throw e;
  }
}
...
export async function fetchDashboardSnapshot(lookbackHours: number): Promise<DashboardSnapshot> {
  ...
  try {
    ...
    if (e?.status === 404) {
      // フォールバック: 旧パス
      return await getJson<DashboardSnapshot>(`/api/dashboard?${q.toString()}`);
    }
    throw e;
  }
}
...
export async function fetchLatestReport(): Promise<AutomationReportSummary> {
  try {
    ...
    if (e?.status === 404) {
      // フォールバック: 旧パス
      return await getJson<AutomationReportSummary>("/api/reports/latest");
    }
    throw e;
  }
}
