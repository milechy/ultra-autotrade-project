...
> Status: Draft v0.3 / 2026-06-26 / 起案: 残高不足時の UX 改善（v0.2: SBI→Base 入金経路 §11.5 / v0.3: §8 金額基準=案a固定・§11.2 有効期限=案A期限分離 を決定）
...
## 8. 再見積もりポリシー & 金額基準【決定: 2026-06-26】

着金には数分〜1h（SBI 経由はブリッジ込みでさらに長い）かかり、その間に状況が変わる。署名直前に：
- **APY / HF after / gas**: 再取得（提案カードの値は参考、実行値は build-tx 時点で再計算）。
- **乖離が大きい場合**（APY 低下/HF 悪化）: 署名前に再確認ダイアログ or 提案を再生成。

### 金額基準【決定: 案a（提案額固定）/ 将来ゴール 案c】
- **✅ 採用（S1/S2）= 案a 提案額固定**: 提案の $X で固定。ユーザーは **$X 以上を入金**し、署名は **$X**。多く入れても投資額は $X、余剰は wallet 残留。予測可能・モデル変更最小。
- **🎯 将来ゴール = 案c ユーザー指定額**: 提案は「戦略の推奨」、ユーザーが投資額を入力（min/max 内）。「利回りが良いから自分で決めて入れる」product 像に最も合致 → S3+ で段階導入。
- 不採用: 案b（着金後実残高×ratio）= 想定外に投資額が増えるリスク・UX 複雑。
...
2. **有効期限 vs 着金ラグ【決定: 案A 期限分離】**: 提案には2つの時計（①市場の鮮度=短命 / ②ユーザーの投資意図=長持ちさせたい）。**✅ 採用 = 案A 期限分離**: 承認＝`awaiting_funds` 化時に **入金期限（funding window）を市場期限と分離**（既定 **7日**・configurable）。市場値（APY/HF/gas）は **署名直前に再見積もり（§8）**、乖離大なら再確認。入金催促は既存 `expiry_reminder_sent_at` を流用。funding window 経過で `expired` ＋「再開しますか？」導線。市場期限（72h `_PROPOSAL_EXPIRES_HOURS`）は `pending`（未承認）にのみ適用し、`awaiting_funds` には funding window を適用する。
