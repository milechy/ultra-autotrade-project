...
### 10.X ドキュメント更新にも適用（2026-04-21追加）

ユーザー向け手順書（tester_onboarding_guide.md / partner_tester_distribution.md /
tester_feedback_template.md 等）を書く・更新する場合、以下を必須とする:

- 書く予定の手順を Playwright E2E で先に実装する
- E2E で「記述通りに操作してユーザーが目的達成できる」ことを確認
- 確認できた手順のみドキュメントに書く
- 確認できていない手順は main マージ禁止

対象例:
- 「X でログイン」→ ログイン → ダッシュボード到達まで E2E で通す
- 「Y ボタンを押して承認」→ 承認成功まで E2E で通す
- 「設定画面で Z を変更」→ 変更が保存されるまで E2E で通す

2026-04-21 教訓: tester docs v2 (PR #111/#112) が E2E 未実装で
Privy 手順を誤記載し main 反映してしまった。E2E (PR #113) を先にやるべきだった。
